...
export const server = {
  ...
  updateScore: defineAction({
    ...
    handler: async (newScore, ctx) => {
      const decodedCookie = ctx.locals.decodedCookie!;
      return await updateScoreServer(decodedCookie, newScore);
    },
  }),
  getScore: defineAction({
    handler: async (_, ctx) => {
      const decodedCookie = ctx.locals.decodedCookie!;
      return await getScoreServer(decodedCookie);
    },
  }),
  updateTheme: defineAction({
    ...
    handler: async (newTheme, ctx) => {
      const decodedCookie = ctx.locals.decodedCookie!;
      return await updateThemeServer(decodedCookie, newTheme);
    },
  }),
  getTheme: defineAction({
    handler: async (_, ctx) => {
      const decodedCookie = ctx.locals.decodedCookie!;
      return await getThemeServer(decodedCookie);
    },
  }),
};
